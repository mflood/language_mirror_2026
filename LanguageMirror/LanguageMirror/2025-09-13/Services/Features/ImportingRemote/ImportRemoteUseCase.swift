...
public final class ImportRemoteUseCase {
    ...
    func run(url: URL, suggestedTitle: String?) async throws -> [Track] {
        ...
        let id = uuid5(namespace: UUID.namespaceDownloadedFile, name: norm(url.absoluteString)).uuidString
        ...
        let folder = lib.trackFolder(forPackId: UUID.namespaceDownloadedFile.uuidString, forTrackId: id)
